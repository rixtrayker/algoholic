...
import { problemsAPI, topicsAPI } from '@/lib/api';
import type { Problem, Topic } from '@/lib/api';
...
export default function ProblemsPage() {
  ...
  const problems = searchQuery.length > 2 ? searchResults?.results || [] : data?.problems || [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600">{problems.length} problems found</div>
      </div>
            ...
            <div>
              ...
              >
                ...
                {topics.map((topic: Topic) => (
                  <option key={topic.topic_id} value={topic.topic_id.toString()}>
                    {topic.name}
                  </option>
                ))}
              </select>
          ...
          problems.map((problem: Problem) => (
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-gray-500">#{problem.problem_id}</span>
                <Link
                  href={`/dashboard/problems/${problem.problem_id}`}
